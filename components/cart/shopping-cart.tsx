...
import { ShoppingCart, X, Plus, Minus, Trash2, Star, Clock, Tag, CreditCard, Shield, ArrowRight } from "lucide-react"
  ...
  return (
    <>
      ...
      <motion.div className="relative" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
        <Button variant="ghost" size="icon" className="hover-glow relative" onClick={() => setIsOpen(true)}>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                  {cartItems.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-500 text-sm">Add some courses to get started!</p>
                    </div>
                  ) : (
                    cartItems.map((item) => (
                      <motion.div
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              <Avatar className="w-5 h-5">
                                ...
                                <AvatarFallback className="text-xs">{item.instructor.charAt(0)}</AvatarFallback>
                              </Avatar>
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-xs text-gray-400">{item.duration}h</span>
                              </div>
                              <Badge variant="outline" className={`text-xs px-1 py-0 ${getLevelColor(item.level)}`}>
                                {item.level}
                              </Badge>
                            </div>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <span className="text-neon-blue font-bold">${item.price}</span>
                ...
                {cartItems.length > 0 && (
                  <div className="p-6 border-t border-white/10 space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {discount > 0 && <p className="text-green-400 text-sm">✅ {discount * 100}% discount applied!</p>}
                    </div>
